...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 gap-4 border bg-background p-5 shadow-lg sm:rounded-lg animate-fadein",
        "max-sm:h-full max-sm:max-w-full max-sm:rounded-none max-sm:overflow-y-auto",
        className
      )}
      {...props}
    >
